        ...
        <?php foreach ($fatura->linhafaturas as $linhafatura){ ?>
            <tr>
                ...
                <?php if($fatura->estado == 0):?>
                    <td><a href="index.php?c=linhafatura&a=delete&id=<?=$linhafatura->id?>" class="btn btn-info" role="button">Eliminar</a></td>
                <?php endif;?>
            </tr>
        <?php } ?>
        </table>
...
<div class="col-sm-6">
    <p>
        <?php if($fatura->estado == 0):?>
            <a href="index.php?c=fatura&a=emitir&id=<?=$fatura->id?>" class="btn btn-info" role="button">Emitir</a>
            <a href="index.php?c=linhafatura&a=create&id=<?=$fatura->id?>" class="btn btn-info" role="button">Nova Linha de Fatura</a>
        <?php else:?>
            <button class="btn btn-info" onclick="printform('form')">Imprimir</button>
        <?php endif;?>
        <a href="index.php?c=fatura&a=index" class="btn btn-info" role="button">Voltar</a>
    </p>
</div>